    let wsmsg = new WebSocket('ws:/172.30.1.19/user');  //채팅용 소켓
    let wslist = new WebSocket('ws:/172.30.1.19/list'); //참여인원 소켓
    ...
    wsmsg.onmessage = async function(event) {
        var eventData = event.data;
            if (event !== null && event !== undefined) {
                ...
                if(msg.time.split(":")[1] < 10){msg.time= msg.time.split(":")[0]+":"+0+msg.time.split(":")[1] }
                if(msg.msg===""){
                    $("#messageBox").append(
